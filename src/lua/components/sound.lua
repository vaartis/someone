...
function M.components.sound_player.process_component(new_ent, comp, entity_name)
   ...
   if comp.volume then sound.sound.volume = comp.volume end
   if comp.loop then sound.sound.loop = comp.loop end
   if comp.position then sound.sound.position = Vector3f.new(comp.position[1], comp.position[2], comp.position[3]) end
